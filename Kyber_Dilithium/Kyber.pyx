...
class Kyber:
    ...
    def keygen(self , seed = None):
        ...
        pk = b''
        for i in range(self.k):
            pk += Encode(plus_mod(self.t._core.datavec[i].nttarray , self.q) , 12)
        pk += rho
        self.pk = pk
        sk = b''
        for i in range(self.k):
            sk += Encode(plus_mod(self.s._core.datavec[i].nttarray , self.q) , 12)
        return pk , sk
    ...
    def cpaenc(self , m , rseed):
        ...
        # enc
        u = KyberVec(0 , self.k , 1)
        r.to_ntt()
        self.A.trans()
        self.A.right_mul(u , r)
        u.to_poly()
        u.add(u , e1)
