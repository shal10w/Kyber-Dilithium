...
void KyberPoly::mul(KyberPoly *res, KyberPoly *b){
    ...
    for(int i = 0 ; i < 128;i++){
        ...
        a1 = this->nttarray[i*2];
        a2 = this->nttarray[i*2+1];
        b1 = b->nttarray[i*2];
        b2 = b->nttarray[i*2+1];
        res->nttarray[i*2] = mod(mod_mul(mod_mul(a2 , b2) , zeta) + mod_mul(a1,b1));
        res->nttarray[i*2+1] = mod(mod_mul(a1 , b2) + mod_mul(a2 , b1));
    }
    res->montflag = 1;
    res->nttflag = 1;
}
